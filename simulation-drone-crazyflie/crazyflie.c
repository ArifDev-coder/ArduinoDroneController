...
int main(int argc, char **argv) {
  ...
  while (wb_robot_step(timestep) != -1) {
    ...
    // desired_state.altitude = 0.0;

    // Controller Custom
    bytes_read = read(serial_fd, serial_buffer, sizeof(serial_buffer) - 1);
    ...
    if (bytes_read > 0) {
      ...
      if (sscanf(serial_buffer, "%d,%d,%d,%d", &throttle, &yaw, &pitch,
                 &roll) == 4) {
        desired_state.altitude = (double)throttle / 255.0 * FLYING_ALTITUDE;
        desired_state.yaw_rate = ((double)yaw - 127.0) / 127.0;
        desired_state.vx = ((double)pitch - 127.0) / 127.0;
        desired_state.vy = ((double)roll - 127.0) / 127.0;

        // printf("%d,%d,%d,%d\n", &throttle, &yaw, &pitch, &roll);
        printf("altitude:%.2f,yaw:%.2f,pitch:%.2f,roll:%.2f \n",
               desired_state.altitude, desired_state.yaw_rate, desired_state.vx,
               desired_state.vy);
      }
    }
    ...
    // Ganti bagian set velocity
    wb_motor_set_velocity(m1_motor, clamp(-motor_power.m1, -600, 600));
    wb_motor_set_velocity(m2_motor, clamp(motor_power.m2, -600, 600));
    wb_motor_set_velocity(m3_motor, clamp(-motor_power.m3, -600, 600));
    wb_motor_set_velocity(m4_motor, clamp(motor_power.m4, -600, 600));
